...
// mine
// best  without sc to store
// during input find max 1 if greater than previous store as ans

// sir lower bound binary search on every row 
...
int RowWithMax(vector<vector<int>>mat , int n, int m){
    int cnt_max=0;
    int index=-1;
    for(int i=0;i<n;i++){
