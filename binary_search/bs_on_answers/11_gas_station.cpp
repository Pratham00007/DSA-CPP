...
// sir brute tc->O(n^2)
// works but time linmit exceeds in test cases
long double mindis(vector<int>arr, int k){
    int n=arr.size();
...
// better
long double mindis2(vector<int>arr, int k){
    int n=arr.size();
    vector<int> howMany(n-1,0);
