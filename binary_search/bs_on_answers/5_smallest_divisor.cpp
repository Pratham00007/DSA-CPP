...
Problem Statement: You are given an array of integers 'arr'
 and an integer i.e. a threshold value 'limit'. 
...
// mine better take min of array as min and max element as max 
// now apply bs and find min

// sir brute go 1 to highest and see when found return it

#include <bits/stdc++.h>
using namespace std;
// sir  and my optimal same bs

int sumBdivisor(vector<int>arr, int div){
    int sum=0;
...
int bs(vector<int>arr, int threshold){
    int low=1;
    int high=*max_element(arr.begin(),arr.end());
    while(low<=high){
        int mid=(low+high)/2;
        if(sumBdivisor(arr,mid)<=threshold){
            high=mid-1;
